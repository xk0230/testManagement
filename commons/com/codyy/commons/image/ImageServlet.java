	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(filename!=null && orginalName!=null){//当源文件名和新文件名都不为null的时候才支持下载操作
		 ...
		 if(filename.contains("headPicDefault")){
			...
			File file = new File(ImageUtil.buildFile(config.getFolder(), filename));
			if (file.exists()) {
				...
				//将谷歌和火狐进行判断 而ie作为默认
				if (isFireFox) {//火狐
					response.setHeader("Content-Disposition", "attachment;filename=\""+new String(orginalName.getBytes("UTF-8"), "ISO-8859-1").replace("&amp;","&")+"\"");
					
				}else if(agent != null && agent.indexOf("AppleWebKit") != -1){//谷歌
					response.setHeader("Content-Disposition", "attachment;filename="+new String(orginalName.getBytes("UTF-8"), "ISO-8859-1").replace("+", "%20").replace("&amp;","&"));	
				} else {//默认ie
					response.setHeader("Content-Disposition", "attachment;filename="+URLEncoder.encode(orginalName, "UTF-8").replace("+", "%20").replace("%26amp%3B", "%26"));
				}
				response.addHeader("Cache-Control", "max-age=864000");
