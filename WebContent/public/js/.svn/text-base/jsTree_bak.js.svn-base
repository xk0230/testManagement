				...
				if (event.which == 1) {
					...
					var moveTrigger = function (event) {
						...
						if (!statrDrag) {
							...
							if (dirY == "down") {//向下拖动
								var $elm = $siblingsTab[nowIndex];
								if ($elm && event.clientY > $elm.offset().top) {//交换位置
									$elm.after($placeHolder);
									nowIndex++;
								}
							} else {//向上拖动 考虑height,width保存
								var $elm = $siblingsTab[nowIndex-1];
								if ($elm && event.clientY < $elm.offset().top + $elm.height()) {
					...
					html.push('<li class="jsTree-node" data-id="' + id + '">\
									<div class="jsTree-content clearfix ' + (options.isExpand?" jsTree-expand ": "") + '" >\
										<label>\
											...
											<input type="checkbox" id="jsTree' + id + '" value="' + id + '" autocomplete="off" ' + (hasCheckBox? '' : ' class="hide" ') + '>'
